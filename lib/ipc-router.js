...
class IPCRouter {
  ...
  emit(endpoint, body) {
    ...
    if (executionEnvironment === 'main') this._sendWebContentsEvent(`ipc-req:${this._oppositeRole}`, { _header: { id: uuid(), endpoint }, body });
    else this._ipc.send(`ipc-req:${this._oppositeRole}`, { _header: { id: uuid(), endpoint }, body });
