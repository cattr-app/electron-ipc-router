...
class IPCRouter {
  ...
  constructor(ipc, id = 'main') {
    ...
    ipc.on(`ipc-req:${this._role}`, async (event, packet) => {
      ...
      // Ignoring packets sent to other router
      if (packet._header.routerId !== this._routerId) return;
  ...
  emit(endpoint, body) {
    ...
    // Parsing url
    const urlDeconstructed = endpoint.split(':');
    endpoint = urlDeconstructed[0];
    const routerId = urlDeconstructed[1] ? urlDeconstructed[1] : 'main';
    ...
    if (executionEnvironment === 'main') this._sendWebContentsEvent(`ipc-req:${this._oppositeRole}`, { _header: { id: uuid(), endpoint, routerId }, body });
    else this._ipc.send(`ipc-req:${this._oppositeRole}`, { _header: { id: uuid(), endpoint, routerId }, body });
  ...
  async request(endpoint, body, timeout = 30000) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const request = { _header: { endpoint, id: uuid(), routerId }, body };
