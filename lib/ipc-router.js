 ...
 * @type {String}
 */
const executionEnvironment = (process.browser || process.type === 'renderer') ? 'renderer' : 'main';
...
class IPCRouter {
  ...
  constructor(ipc, id = 'main') {
    ...
    if (typeof ipc !== 'object')
      throw new Error('Incorrect IPC interface given');
    ...
    ipc.on(`ipc-req:${this._role}:${this._routerId}`, async (event, packet) => {
      ...
      if (typeof packet._header !== 'object' || typeof packet._header.endpoint !== 'string' || typeof packet._header.id !== 'string')
        return;
      ...
      request.send = (...args) => {
        ...
        if (args.length === 0)
          throw new Error('Incorrect call of request.send function');
        ...
        if (typeof code !== 'number' || typeof body !== 'object')
          throw new TypeError('Incorrect arguments types in request.send function');
    ...
    this._sendWebContentsEvent = (event, message) => {
      ...
      if (typeof event !== 'string' || typeof message === 'undefined')
        throw new TypeError('Incorrect event name or payload');

      // Checking is WebContents instance already set in this IPC instance
      if (this._webcontent)
        throw new Error('WebContents instance is not linked into this IPC instance');
     ...
     */
    this._createTimeoutDestructor = (packetId, timeout) => setTimeout(() => {

      // Checking is this request exists
      if (typeof this._requestsPool[packetId] === 'undefined')
        return;
  ...
  setWebContents(wc) {
    ...
    if (executionEnvironment === 'renderer')
      return false;

    // Checking argument type
    if (typeof wc !== 'object')
      throw new TypeError('Incorrect WebContent instance passed');
  ...
  serve(endpoint, handler, override = false) {
    ...
    if (typeof endpoint !== 'string' || typeof handler !== 'function')
      throw new TypeError('Trying to register incorrect endpoint or handler');

    // Checking is this endpoint already have a handler?
    if (!override && typeof this._routeHandlers[endpoint] !== 'undefined')
      throw new Error(`Restricted attempt to override handler for endpoint: ${endpoint}`);
  ...
  emit(endpoint, body) {
    ...
    if (typeof endpoint !== 'string' || typeof body !== 'object')
      throw new TypeError('Trying to emit incorrect endpoint or body');

    // Choosing appropriate function based on the current execution environment
    if (executionEnvironment === 'main')
      this._sendWebContentsEvent(`ipc-req:${this._oppositeRole}:${this._routerId}`, { _header: { id: nanoid(), endpoint }, body });
    else
      this._ipc.send(`ipc-req:${this._oppositeRole}:${this._routerId}`, { _header: { id: nanoid(), endpoint }, body });
  ...
  async request(endpoint, body, timeout = 30000) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (typeof endpoint !== 'string' || typeof body !== 'object') {
        ...
        reject(new TypeError('Trying to request incorrect endpoint or body'));
        return;
      ...
      if (typeof timeout !== 'number' || timeout < 0) {
        ...
        reject(new TypeError(`Incorrect packet timeout: ${timeout}`));
        return;

      }

      // Create request object
      const request = { _header: { endpoint, id: nanoid() }, body };
      ...
      if (executionEnvironment === 'renderer')
        this._ipc.send(`ipc-req:${this._oppositeRole}:${this._routerId}`, request);
      else
        this._sendWebContentsEvent(`ipc-req:${this._oppositeRole}:${this._routerId}`, request);
